...
const getPopularMedia = async (req, res) => {
    ...
    const apiUrlMap = {
        movie: `${TMDB_BASE_URL}/movie/popular`,
    ...
    try {
        ...
        console.error("Error fetching popular TV results from TMDB:", error.message);
        res.status(500).json({ error: "Failed to fetch popular TV results from TMDB." });
    }
};
